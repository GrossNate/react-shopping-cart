...
import { cartItemAddedSchema, cartItemsSchema, CatalogItem, catalogItemSchema, catalogItemsSchema, NewItem } from '../../types';
...
export const addItem = async (productId: Pick<CatalogItem, "_id">) => {
  const { data } = await axios.post('/api/add-to-cart', { productId: productId._id });
  if (data) {
